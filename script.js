...
(() => {
  ...
  // Self-contained mobile audio. No MP3 files are required.
  ...
  function startMusic() {
    ...
    const ctx = ensureAudioContext();
    if (!ctx) {
      showToast('Your browser has sound disabled, but the story still works perfectly.');
      return;
    }
    if (ambientNodes.length) return;
    const master = ctx.createGain();
    master.gain.setValueAtTime(.0001, ctx.currentTime);
    master.gain.exponentialRampToValueAtTime(.012, ctx.currentTime + 1.2);
    ...
    playAmbientAccent();
    ambientTimer = setInterval(playAmbientAccent, 5200);
  ...
  function toggleMusic() {
    if (!state.musicOn) {
      state.musicOn = true;
      startMusic();
      showToast('Soft cinematic sound on');
    } else {
      state.musicOn = false;
      stopMusic();
      showToast('Music off');
  ...
  function replayStory() {
    stopMusic(); state.musicOn=false; state.currentScene=0.5; state.noClicks={scene4:0,scene17:0}; state.complaintSubmitted=false; state.firstLetterOpened=false; state.finalLetterOpened=false; state.surpriseOpened=false;
